...
import com.mvproject.tinyiptvkmp.features.playlist.api.data.repository.PlaylistSyncStateRepositoryImpl
...
val playlistApiModule =
    module {
        ...
        single<PlaylistSyncStateRepository> { PlaylistSyncStateRepositoryImpl(get()) }
        single<SelectedPlaylistProvider> { SelectedPlaylistProviderImpl(get()) }
        single { PlaylistContentUpdater(get(), get()) }

        single<GetPlaylistUseCase> { GetPlaylistUseCaseImpl(get()) }
        single<ObservePlaylistsUseCase> { ObservePlaylistsUseCaseImpl(get()) }
        single<CreatePlaylistUseCase> { CreatePlaylistUseCaseImpl(get(), get(), get()) }
        single<UpdatePlaylistUseCase> { UpdatePlaylistUseCaseImpl(get(), get(), get()) }
        ...
        single<UpdateRemotePlaylistChannelsUseCase> {
            UpdateRemotePlaylistChannelsUseCaseImpl(get(), get(), get())
        }
    }
